...
let test_is_report_safe_with_tolerance_two_equal_numbers () =
  (check bool) "when given [ 1; 1; 3; 6; 7; 9 ]" true
    (D2.is_report_safe_with_tolerance [ 1; 1; 3; 6; 7; 9 ])

let test_is_report_safe_with_tolerance_fault_in_first () =
  (check bool) "when given [ 7; 1; 3; 6; 7; 9 ]" true
    (D2.is_report_safe_with_tolerance [ 7; 1; 3; 6; 7; 9 ])
    ...
    [
      ...
      ( "Part 2",
        [
          ...
          ( "Checks report safety correctly with tolerance and the first two \
             numbers being the same",
            `Quick,
            test_is_report_safe_with_tolerance_two_equal_numbers );
          ( "Checks report safety correctly with tolerance and fault in first \
             level",
            `Quick,
            test_is_report_safe_with_tolerance_fault_in_first );
